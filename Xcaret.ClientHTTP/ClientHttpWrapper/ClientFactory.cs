...
namespace Xcaret.ClientHTTP.ClientHttpWrapper
{
    /// <summary>
    ...
    public class ClientFactory : IClientFactory
    {
            ...
            SerializeFormat serializeFormat)
        {
            ...
            //var headersLogR = string.Empty;
            //if (headers != null
            //    && headers.Any())
            //{
            //    //httpClient.DefaultRequestHeaders.Authorization
            //    //         = new AuthenticationHeaderValue("Basic", headers["Authorization"]);
            //    //headersLogR = string.Join(",", headers.Select(k => $"{k.Key}, {k.Value}"));
            //}

            //Serilog.Log.Error($"Url: {url} | Headers: [{headersLogR}]");

            var result = await httpClient.SendAsync(requestGet);//await httpClient.GetAsync(url);
        ...
        {
            return serializeFormat == SerializeFormat.Json ?
